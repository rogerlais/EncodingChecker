...
namespace UtfUnknown {
	...
	public class CharsetDetector {
		...
		protected virtual void Feed(byte[] buf, int offset, int len) {
			...
			foreach (var prober in this.CharsetProbers) {
				this._done = this.RunProber( buf, offset, len, prober );
				if (this._done)
					return;
			}
		}
		...
		private bool IsStartsWithBom(byte[] buf, int offset, int len) {
			...
			if (bomSet != null) {
				this._detectionDetail = new DetectionDetail( bomSet, 1.0f ) {
					HasBOM = true
				};
				return true;
			}
		...
		private DetectionResult DataEnd() {
			...
			if (this._detectionDetail != null) {
				...
				this._detectionDetail.Confidence = 1.0f;
				return new DetectionResult( this._detectionDetail );
			}

			if (this.InputState == InputState.Highbyte) {
				var detectionResults = this._charsetProbers
			...
			} else if (this.InputState == InputState.EscASCII) {
				return new DetectionResult( new DetectionDetail( CodepageName.ASCII, 1.0f ) );
			}
